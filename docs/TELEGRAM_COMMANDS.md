...
- when you switch sessions, only the user inputs from the last 2 hours of session history before the latest message in that session plus the latest Codex summary from the session are mirrored back into Telegram as display-only chat output
