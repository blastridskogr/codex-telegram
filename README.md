...
- replays only the user inputs from the last 2 hours of session history plus the latest final Codex summary from the session when you switch sessions
...
- Session switching replays only the user inputs from the last 2 hours of session history before the latest message in that session, plus the latest final Codex summary from the session, back to Telegram as display-only chat messages. On the tested build that final summary comes from `task_complete.last_agent_message`. This does not spend extra model tokens.
