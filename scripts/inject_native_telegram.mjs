...
function upsertMainIpcTelegramResultHandlers(source) {
  ...
  const existingHandlersPattern = new RegExp(
    `if\\(${escapeRegex(payloadVar)}\\.type===\\\`telegram-start-conversation-result\\\`\\)\\{[\\s\\S]*?if\\(${escapeRegex(payloadVar)}\\.type===\\\`telegram-respond-approval-result\\\`\\)\\{[\\s\\S]*?return\\}`
  );
...
st=async(e,n=null,r=750)=>{let i=at(it());ot(i,{type:\`navigate-to-route\`,path:e,state:n});await new Promise(e=>setTimeout(e,r));return i;},
  ...
  if (!renderer.includes("globalThis.__codexTelegramApprovalController=")) {
    ...
    if (renderer.includes(approvalPresenceAnchor)) {
      renderer = replaceOnce(
        ...
        "globalThis.__codexTelegramApprovalController=ae?.type===`approval`?globalThis.__codexTelegramApprovalController??null:null;let ce=gt(k),le=_?.trim()??``,ue=v?.trim()??``,de=ce?.pendingSteers??[],fe=od(),pe=SQ(ce,k!=null&&Et.getSessionForConversation(k)!=null),{memberships:he,rows:ge,mentionItems:ve,firstApproval:ye}=Xme({activeConversationId:k,conversation:ce,enabled:fe,manager:A}),be=ye!=null,Se=xt(ae),Ce=ae?.type===`approval`||be,we=k!=null&&(Se||be),[Te,Ee]=(0,Z.useState)(!1),De=q(pX),Oe=ge.length>0&&!Ce&&!De&&!Te,[Ae]=aq(),{isRequired:je}=rq(),{isPending:Ne}=pq(A),",
